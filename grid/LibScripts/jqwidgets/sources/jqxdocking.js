    ...
    $.extend($.jqx._jqxDocking.prototype, {
        ...
        _renderWindows: function (panel) {
            ...
            for (var j = 0; j < windows.length; j += 1) {
                ...
                $(windows[j]).jqxWindow({rtl: this.rtl, theme: this.theme, enableResize: false, width: $(windows[j]).css('width'), maxWidth: Number.MAX_VALUE });
                $(windows[j]).detach();
        ...
        _addEventListeners: function () {
            ...
            var me = this;
            $(window).on('resize.' + this.element.id, function (event) {
                me._performLayout();
            });
        },
        ...
        addWindow: function (window, mode, panel, position) {
            var selector = '#' + window;
            $(selector).jqxWindow({rtl: this.rtl, theme: this.theme, enableResize: false, width: $(selector).css('width'), maxWidth: Number.MAX_VALUE });
