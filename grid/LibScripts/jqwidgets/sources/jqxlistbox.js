...
(function ($) {
    ...
    $.extend($.jqx._jqxListBox.prototype, {
        ...
        render: function () {
            ...
            this.vScrollBar = verticalScrollBar.jqxScrollBar({ 'vertical': true, rtl: this.rtl, theme: this.theme, touchMode: this.touchMode, largestep: parseInt(this.host.height()) / 2 });
        ...
        _updateTouchScrolling: function () {
            ...
            if (this.isTouchDevice()) {
                ...
                $(element).off($.jqx.mobile.getTouchEventName('touchstart') + '.touchScroll');
                $(element).off($.jqx.mobile.getTouchEventName('touchmove') + '.touchScroll');
                $(element).off($.jqx.mobile.getTouchEventName('touchend') + '.touchScroll');
                $(element).off('touchcancel.touchScroll');
        ...
        _addItems: function (refreshUIItems) {
            ...
            for (var virtualItemIndex = startIndex; virtualItemIndex < this.virtualItemsCount; virtualItemIndex++) {
                ...
                var itemElement = $("<div style='-ms-touch-action: none; border: none; width:100%; height: 100%; align:left; valign:top; position: absolute;'><span style='-ms-touch-action: none;' id='spanElement'></span></div>");
                ...
                if (this.checkboxes && this.host.jqxCheckBox) {
                    var checkbox = $('<div style="background-color: transparent; padding: 0; margin: 0; position: absolute; float: left; width: 16px; height: 16px;" class="chkbox"/>');
        ...
        unselectIndex: function (index, render) {
            ...
            if (render == undefined || render == true) {
                this._renderItems();
                this._raiseEvent('1', { index: index });
            }
            ...
            if (this.fromSelect == undefined || this.fromSelect == false) {
                for (var indx = 0; indx < length; indx++) {
                    ...
                    if (indx == 0 && group.caption == '' && group.captionHtml == '' && length <= 1) {
                        ...
                        self.itemsByValue[$.trim(key).split(" ").join("")] = listBoxItem;
