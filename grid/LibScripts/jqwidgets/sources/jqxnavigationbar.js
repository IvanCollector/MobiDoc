...
(function ($) {
    ...
    $.extend($.jqx._jqxNavigationBar.prototype, {
        ...
        defineInstance: function () {
            ...
            this.initContent; // callback function
        ...
        expandAt: function (index) {
            ...
            if (selectedItem.disabled == false && selectedItem.expanded == false && selectedItem._expandChecker == 1) {
                ...
                selectedItem._arrow.removeClass(this.toThemeProperty("icon-arrow-down"));
                selectedItem._arrow.removeClass(this.toThemeProperty("jqx-expander-arrow-top"));
                selectedItem._arrow.addClass(this.toThemeProperty("icon-arrow-up"));
                        ...
                        content.animate(showProps,
                            {
                                ...
                                complete: function () {
                                    ...
                                    selectedItem._header.attr("aria-expanded", true);
                                    selectedItem._content.attr("aria-hidden", false);
        ...
        collapseAt: function (index) {
            ...
            if (selectedItem.disabled == false && selectedItem.expanded == true && selectedItem._expandChecker == 0) {
                ...
                selectedItem._arrow.removeClass(this.toThemeProperty("icon-arrow-up"));
                ...
                selectedItem._arrow.addClass(this.toThemeProperty("icon-arrow-down"));
                    ...
                    case 'slide':
                        ...
                        content.animate(hideProps, {
                            ...
                            complete: function () {
                                ...
                                content.hide();
                                selectedItem._header.attr("aria-expanded", false);
                                selectedItem._content.attr("aria-hidden", true);
                                me._updateExpandedIndexes();
        ...
        render: function () {
            ...
            var me = this;
    //        this.host.html(this._cachedHTMLStructure);
            ...
            $.each(this.items, function (index, value) {
                ...
                if (item.expanded == true) {
                    item._arrow.addClass(me.toThemeProperty("icon-arrow-up"));
                    ...
                    item._header.attr("aria-expanded", true);
                    item._content.attr("aria-hidden", false);
                } else if (item.expanded == false) {
                    item._arrow.addClass(me.toThemeProperty("icon-arrow-down"));
                    ...
                    item._header.attr("aria-expanded", false);
                    item._content.attr("aria-hidden", true);
                };
            });
        ...
        insert: function (index, header, content) {
            ...
            var newItemHTML = "<div><div>" + header + "</div><div></div></div><div>" + content + "</div>";
            if (index != -1) {
                $(newItemHTML).insertBefore(this.items[index]._header);
                newItem._header = this.host.children("div:eq(" + index * 2 + ")");
                newItem._content = this.host.children("div:eq(" + (index * 2 + 1) + ")");
            } else {
                var lastIndex = this.items.length - 1;
                $(newItemHTML).insertAfter(this.items[lastIndex]._content);
                newItem._header = this.host.children("div:eq(" + (lastIndex * 2 + 2) + ")");
                newItem._content = this.host.children("div:eq(" + (lastIndex * 2 + 3) + ")");
                index = lastIndex;
            };

            if (newItem._header.attr("tabindex") == undefined) {
                this.tI++;
                newItem._header.attr("tabindex", this.tI);
            };
            newItem._header_text = newItem._header.children("div:eq(0)");
            newItem._arrow = newItem._header.children("div:eq(1)");
            newItem.expandedFlag = false;
            newItem.expanded = false;
            newItem.disabled = false;
            newItem._arrow.addClass(this.toThemeProperty("icon-arrow-down"));
            newItem._arrow.addClass(this.toThemeProperty("jqx-expander-arrow-top"));
            newItem._initialized = false;
            newItem._expandChecker = 1;
            newItem._content.css("display", "none");
        ...
        destroy: function () {
            this.host.remove();
            $(this.element).removeData('jqxNavigationBar');
        },
        ...
        _setSize: function () {
            ...
            $.each(this.items, function (index, value) {
                ...
                item._header.width(me.width);
                var borderOffset = 2;
                item._header.width(item._header.width() - borderOffset);
                var padding = parseInt(item._header_text.css('padding-left'));
                if (me.rtl) {
                    var padding = parseInt(item._header_text.css('padding-right'));
                }
                if (isNaN(padding)) padding = 0;

                if (me.showArrow == true && me.width != "auto") {
                    item._header_text.width(item._header.width() - item._arrow.width() - padding);
                } else {
                    item._header_text.width("auto");
                };
                item._content.width(me.width);
                item._content.width(item._header.width());
            ...
            if (this._isTouchDevice == false) {
                ...
                if (this.toggleMode != "none") {
                    $.each(this.items, function (index, value) {
                        var item = this;
                        if (item.disabled == false) {
                            me.addHandler(item._header, 'touchstart.navigationbar' + this.widgetID, function () {
                                me._animate(index);
                            });
                        };
                    });
        ...
        _removeHandlers: function () {
            ...
            $.each(this.items, function (index, value) {
                var item = this;
                me.removeHandler(item._header, 'click.navigationbar' + this.widgetID);
                me.removeHandler(item._header, 'dblclick.navigationbar' + this.widgetID);
                me.removeHandler(item._header, 'mouseenter.navigationbar' + this.widgetID);
                me.removeHandler(item._header, 'mouseleave.navigationbar' + this.widgetID);
                me.removeHandler(item._header, 'focus.navigationbar' + this.widgetID);
                me.removeHandler(item._header, 'blur.navigationbar' + this.widgetID);
                me.removeHandler(item._content, 'focus.navigationbar' + this.widgetID);
                me.removeHandler(item._content, 'blur.navigationbar' + this.widgetID);
            });
        },
        ...
        _setTheme: function () {
            ...
            $.each(this.items, function (index, value) {
                ...
                if (item.disabled == false) {
                    ...
                    me.addHandler(item._header, 'mouseenter.navigationbar' + this.widgetID, function () {
                    ...
                    me.addHandler(item._header, 'mouseleave.navigationbar' + this.widgetID, function () {
        ...
        _focus: function () {
            ...
            $.each(this.items, function (index, value) {
                ...
                me.addHandler(item._header, 'focus.navigationbar' + this.widgetID, function () {
                    item.focusedH = true;
                    item._header.attr("aria-selected", true);
                ...
                me.addHandler(item._header, 'blur.navigationbar' + this.widgetID, function () {
                    item.focusedH = false;
                    item._header.attr("aria-selected", false);
