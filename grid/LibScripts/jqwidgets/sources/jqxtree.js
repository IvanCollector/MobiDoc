    ...
    $.extend($.jqx._jqxTree.prototype, {
        ...
        _initialize: function (mode, oldmode) {
            ...
            $.each(this.items, function () {
                ...
                $arrowSpan.on(eventName, function () {
                    if (!item.isExpanded) {
                        me._expandItem(me, item);
                    }
                    else {
        ...
        _render: function (updateEvents, updateDrag) {
            ...
            if (this.host.jqxPanel) {
                ...
                panel.jqxPanel({ rtl: this.rtl, theme: this.theme, width: this.width, height: this.height, autoUpdateInterval: 30, touchMode: this.touchMode, autoUpdate: true, sizeMode: sizeMode });
                if (this.height == null || (this.height != null && this.height.toString().indexOf('%') != -1)) {
                    if (this.isTouchDevice()) {
                        panel.off($.jqx.mobile.getTouchEventName('touchend') + '.touchScroll touchcancel.touchScroll');
                        panel.off($.jqx.mobile.getTouchEventName('touchmove') + '.touchScroll');
                        panel.off($.jqx.mobile.getTouchEventName('touchstart') + '.touchScroll');
                    }
                }
            ...
            if (!hasTitleAttribute || !isSameLI) {
                if ($(listTag.firstChild).length > 0) {
                    ...
                    if ($.jqx.browser.msie && $.jqx.browser.version < 8) {
                        ...
                        this._measureItem.html($(treeItem.titleElement).text());
                        var width = 5 + this._measureItem.outerWidth();
                        if (treeItem.icon) {
                            width += 20;
                        }
                        if (this.checkboxes) {
