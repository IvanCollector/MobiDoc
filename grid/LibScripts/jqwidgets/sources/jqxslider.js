...
(function ($) {
    ...
    $.extend($.jqx._jqxSlider.prototype, {
        ...
        render: function () {
            ...
            this._addInput();
            this.host.attr('aria-valuemax', this.max);
            this.host.attr('aria-valuemin', this.min);
            this.host.attr('aria-valuenow', this.value);
        },
        ...
        _addTick: function (container, position, value) {
            var currentTick, size = container[this._getSetting('oSize')]();
            if (this.orientation === 'horizontal') {
            ...
            currentTick[this._getSetting('oSize')](this.tickSize);
        },

        _tickLayout: function (container, currentTick) {
            var size = container[this._getSetting('oSize')]();
            if (container[0] === this._topTicks[0]) {
                currentTick.css(this._getSetting('top'), '2px');
            } else {
                currentTick.css(this._getSetting('top'), size - this.tickSize - 2);
            }
        },
        ...
        _trackMouseDownHandler: function (event) {
            ...
            var self = event.data.self,
                ...
                position = self._track.offset()[self._getSetting('left')],
        ...
        _getClosest: function (position) {
            if (!this.rangeSlider) {
                ...
                position = position - this._track.offset()[this._getSetting('left')] - this._slider.left[this._getSetting('size')]() / 2;
        ...
        _startDrag: function (event) {
            ...
            self._mouseStartX = (self._isTouchDevice) ? touch.pageX : event.pageX;
            self._mouseStartY = (self._isTouchDevice) ? touch.pageY : event.pageY;
            self.focus();
            return false;
        },
        ...
        _stopDrag: function () {
            ...
            if (this.input) {
                this.host.attr('aria-valuenow', this.input.val());
            }
            self._slider.left.removeClass(self.toThemeProperty('jqx-fill-state-pressed'));
            self._slider.right.removeClass(self.toThemeProperty('jqx-fill-state-pressed'));
        ...
        _performDrag: function (event) {
            ...
            if (self._capturedElement !== null) {
                ...
                self._isDragged(event[self._getSetting('page')])
                if (self._slideStarted) {
                    ...
                    return self._dragHandler(event[self._getSetting('page')]);
                }
            }
        },
        ...
        _getFloatingValueByPosition: function (position) {
            var relativePosition = position - this._track.offset()[this._getSetting('left')] + this._slider.left.width() / 2,
        ...
        _getFixedValueByPosition: function (position) {
            var step = this.step,
                ...
                currentSectorPosition = this._track.offset()[this._getSetting('left')] - this._slider.left[this._getSetting('size')]() / 2,
                closestSector = { number: -1, distance: Number.MAX_VALUE };
            //position -= this._track.offset()[this._getSetting('left')];
