    ...
    $.extend($.jqx._jqxWindow.prototype, {
        ...
        _addDragDropHandlers: function () {
            if (this.draggable) {
                ...
                this.addHandler(this._header, 'blur', function () {
        ...
        _headerMouseDownHandler: function (self, event) {
            ...
            if (self._resizeDirection == null) {
                ...
                self._mousePosition.x = touch.pageX;
                self._mousePosition.y = touch.pageY;
...
var resizeModule = (function ($) {
    return {
        ...
        _removeResizeEventListeners: function () {
            ...
            this._resizeWrapper.off('mousemove.resize' + resizetargetid);
            this._resizeWrapper.off('mousedown.resize' + resizetargetid);
            $(document).off('mousemove.resize' + resizetargetid);
            $(document).off('mouseup.resize' + resizetargetid);
        },

        _addResizeEventHandlers: function () {
            var resizetargetid = this.resizeTarget.attr('id');
            var self = this;
            this._resizeWrapper.on('mousemove.resize.' + resizetargetid, { self: this }, function (event) {
                self._resizeCursorChangeHandler(self, event);
            });
            this._resizeWrapper.on('mousedown.resize.' + resizetargetid, { self: this }, function (event) {
                self._resizeMouseDownHandler(self, event);
            });
            $(document).on('mousemove.resize.' + resizetargetid, { self: this }, function (event) {
                self._resizeHandler(self, event);
            });
            $(document).on('mouseup.resize.' + resizetargetid, { self: this }, function (event) {
