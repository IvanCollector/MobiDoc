...
(function ($) {
    ...
    $.extend($.jqx._jqxMenu.prototype, {
        ...
        createInstance: function (args) {
            ...
            $(window).on('resize.menu' + this.element.id, function () {
                if (percentageSize) {
                    if (me.refreshTimer) {
        ...
        _applyOrientation: function (mode, oldmode) {
            ...
            if (this.autoOpenPopup) {
                if (this.mode == 'popup') {
                    ...
                    $(document).on('mousedown.menu' + this.element.id, me, this._openContextMenu);
                }
                else {
                    $(document).off('contextmenu');
                    $(document).off('mousedown.menu' + this.element.id, this._openContextMenu);
                }
            }
            else {
                $(document).off('contextmenu');
       ...
       //         this.addHandler($(document), 'mouseup.menu' + this.element.id, me._closeAfterClick, me);
        ...
        propertyChangedHandler: function (object, key, oldvalue, value) {
            ...
            if (key == 'autoCloseOnClick') {
                if (value == false) {
                    $(document).off('mousedown.menu' + this.element.id, object._closeAll);
                }
                else {
                    $(document).on('mousedown.menu' + this.element.id, object, object._closeAll);
                }
            }
